...
export async function GET(request: Request) {
  ...
  const sort = searchParams.get("sort") || "fecha"; // Which column to sort by (defaults to 'fecha')
  ...
  // If the user is searching for something, validate the search format
  if (search) {
    const isNumeric = /^\d+$/.test(search);
    const uuidRegex =
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

    // If search doesn't match UUID or numeric format, return empty results immediately
  ...
  // Start building our database query to get orders
  let query = supabase.from("Pedidos").select("*", { count: "exact" });

  // Apply metodo filter (if one or more selected)
  if (metodos.length > 0) {
